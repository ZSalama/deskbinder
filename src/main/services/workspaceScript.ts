...
import type { RepoSettings, WorkspaceScriptResult } from '../../shared/deskbinder'
...
async function validateScriptPath(scriptPath: string): Promise<string> {
  const resolvedScriptPath = normalize(await realpath(scriptPath))
...
function resolveWorkspaceScriptPath(repo: RepoSettings): string {
  const configuredPath = sanitizeString(repo.workspaceScriptPath) ?? DEFAULT_WORKSPACE_SCRIPT_PATH

  return normalize(
    isAbsolute(configuredPath) ? configuredPath : resolve(repo.repoPath, configuredPath)
  )
}
...
export async function runWorkspaceScript({
  ...
  try {
    ...
    const executablePath = await validateScriptPath(resolveWorkspaceScriptPath(repo))
