...
Deskbinder creates new workspaces by running a repo-owned executable script from the desktop app's main process. The default configured script path is `new_workspace`, relative to the parent repository root.

This document describes the contract the script must satisfy for Deskbinder to create, register, use, and later delete a workspace successfully.

## Purpose

The workspace script is responsible for preparing a new local Git worktree for a requested branch. Deskbinder does not create the worktree itself. It validates the script location, invokes the script, parses its final JSON result, then registers the returned worktree as a runnable workspace.
...
4. For each successful script result, Deskbinder validates the returned `workspacePath` as a Git worktree.
5. Deskbinder registers the worktree as a new repo entry linked back to the parent repository.
6. Deskbinder records any returned process PIDs for later cleanup.
...
Deskbinder invokes the script directly with `spawn`, not through a shell:
...
- Batch creation supports 1 to 5 workspaces; Deskbinder runs the script sequentially once per requested branch.

Branch names are validated by Deskbinder with:
...
For Deskbinder to register the created workspace, all of the following must be true:
...
Deskbinder validates the returned `workspacePath` by resolving the real path, checking `git rev-parse --show-toplevel`, and requiring `git rev-parse --git-dir` to differ from `git rev-parse --git-common-dir`.

If the script returns `ok: true` but omits `agentRunnable: true` or `workspacePath`, Deskbinder will not register a new workspace and will keep the selected repo on the source repository.
...
Deskbinder first tries to parse all trimmed stdout as JSON. If that fails, it tries to parse the final JSON object beginning at the last `{` in stdout. Because that fallback is intentionally simple, avoid printing other JSON objects to stdout before the result.
...
- `agentRunnable`: Must be exactly `true` for Deskbinder to register the workspace for agent runs.
- `workspacePath`: Required for registration. Prefer an absolute path to the created worktree root.
- `workspaceName`: Optional display name for the new workspace. If omitted, Deskbinder uses the basename of `workspacePath`.
- `branchName`: Optional normalized branch name. If omitted, Deskbinder uses the requested branch name.
- `repoPath`: Optional parent repo path. If omitted, Deskbinder uses the configured source repo path.
...
- `processes.dev`, `processes.convex`: Optional numeric PIDs for long-running processes Deskbinder should track and clean up.
...
Deskbinder also creates its own failures for:
...
For non-zero exits, Deskbinder surfaces trimmed stderr, up to 2000 characters.
...
- The branch name returned in JSON matches the actual branch Deskbinder should display and later delete.

Deskbinder's delete workflow assumes a registered workspace is a Git worktree. It removes the worktree, prunes worktree metadata, and then deletes the associated branch from the source repository.
...
For typical Deskbinder workspaces, the script should copy local runtime env files into the worktree after it is created:
...
Deskbinder only tracks returned PIDs that:
...
# Keep stdout reserved for Deskbinder's final JSON result.
exec 3>&1
exec 1>&2
...
workspace_root="${DESKBINDER_WORKSPACE_ROOT:-$default_workspace_root}"
safe_name="$(
  printf '%s' "$branch_name" \
    | tr '[:upper:]_' '[:lower:]-' \
    | sed -E 's#[^a-z0-9-]+#-#g; s#^-+##; s#-+$##; s#-+#-#g'
...
export DESKBINDER_BASE_BRANCH="$base_branch"
export DESKBINDER_WORKSPACE_NAME="$(basename "$workspace_path")"
